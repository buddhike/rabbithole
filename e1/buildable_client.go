...
func (b *BuildableClient) build() {
	...
	if b.metricWriter != nil {
		b.client = wrapWithTracingRoundtripper(b.client, b.metricWriter)
	}
}
...
func (b *BuildableClient) clone() *BuildableClient {
	...
	cpy.metricWriter = b.metricWriter
...
func (b *BuildableClient) WithMetricWriter(mw metrics.WriteAPI) *BuildableClient {
	cpy := b.clone()
	cpy.metricWriter = mw
	return cpy
}
...
func wrapWithTracingRoundtripper(c *http.Client, mw metrics.WriteAPI) *http.Client {
	tr := c.Transport
	if tr == nil {
		tr = defaultHTTPTransport()
	}
	...
	cc := *c
	cc.CheckRedirect = limitedRedirect
	cc.Transport = &TracingRoundTripper{
		inner: tr,
		mw:    mw,
	}
